class AgenteModelo extends AgenteBase {
    ...
    executar(percepcoes, estado) {
        ...
        // 2. Anti-loop: se parado por 5+ passos, força andar
            if (this.vezesParado > 5) {
        this.vezesParado = 0;
        // Tenta qualquer direção que esteja dentro do mapa
        for (let i = 0; i < 4; i++) {
            const d = (this.direcao + i) % 4;
            const nx = estado.x + this.dx[d];
            const ny = estado.y + this.dy[d];
            if (this.isDentro(nx, ny, estado.tamanho)) {
                // Vira para essa direção e anda
                const diff = (d - estado.orientacao + 4) % 4;
